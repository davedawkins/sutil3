...
type CleanCommand() =
    inherit
        CommandFn(fun _ ->
            Command.Run("dotnet", "fable clean --yes -o fable-build")
            Command.Run("dotnet", "clean")
        )

type PublishSutilPackageCommand() =
    inherit
        CommandFn(fun _ ->
            let pkg = "Sutil"
...
type PublishSutilHtmlPackageCommand() =
    inherit
        CommandFn(fun _ ->
            let pkg = "Sutil.Html"
            pushNuget ("src" </> pkg </> pkg + ".fsproj") [] doNothing
...
type SutilXmlCommand() =
    inherit
        CommandFn(fun _ ->
            Command.Run("dotnet", "build -c Release src/Sutil/Sutil.fsproj")

            Utils.copyFileOverwrite
                "src/Sutil/bin/Release/netstandard2.0/Sutil.xml"
                "public/Sutil.xml"
        )
...
module FsDocsCommand =
    ...
    let fsdocs (local: bool) =
        ...
        let root =
            if local then
                "http://127.0.0.1:5500/public/apidocs/"
            else
                "https://sutil.dev/apidocs/"
        ...
        Command.Run(
            "dotnet",
            sprintf
                "fsdocs build --input %s --output public/apidocs --parameters fsdocs-logo-src ../../images/logo-small.png root %s"
                docsInputFolder
                root
        )
...
let main args =
    ...
    app.Configure(fun config ->
        ...
        config
            .AddCommand<PublishSutilPackageCommand>("publish:sutil")
            .WithDescription("Build Sutil package and push to nuget")
        |> ignore

        config
            .AddCommand<PublishSutilHtmlPackageCommand>("publish:sutil:html")
            .WithDescription("Build Sutil.Html package and push to nuget")
        |> ignore
