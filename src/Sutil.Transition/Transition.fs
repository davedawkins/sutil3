...
let internal showEl (el : HTMLElement) isVisible =
    ...
    let ev = CustomEvents.customEvent (if isVisible then CustomEvents.SHOW else CustomEvents.HIDE) {|  |}
...
let private overrideDuration d = if slowAnimations then 10.0 * d else d
let private overrideDurationFn fo = if slowAnimations then (fo |> Option.map (fun f -> ((*)10.0 << f))) else fo
...
let private runTasks() =
    ...
    if (copy.Length > 0) then
        if log.enabled then log.info($"- - - Tasks: running {copy.Length} tasks - - - - - - - - - - - - - -")
    for f in copy do f()
...
let createRule (node : HTMLElement) (a:float) (b:float) tr (uid:int) =
    ...
    if log.enabled then log.info( sprintf "keyframe: %s" (keyframes |> List.skip (keyframes.Length / 2) |> List.head))
    if log.enabled then log.info($"createRule {name} {durn}ms for {nodeStr node}")
    ...
    Timers.rafu( fun _ ->
        if (numActiveAnimations = 0) then
            for kv in activeDocs do
                ...
                if log.enabled then log.info(sprintf "clearing %d rules" (int stylesheet.cssRules.length))
...
let flip props (node:Browser.Types.Element) (animation:Animation) =
    ...
    if log.enabled then log.info( sprintf "flip: %A,%A %A %A -> %A" dx dy transform (rectToStr animation.From) (rectToStr animation.To))
    {
        tr with
