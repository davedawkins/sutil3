...
module SvgCanvas =
    ...
    let zip3 (a: IObservable<'a>) (b: IObservable<'b>) (c: IObservable<'c>) =
        Observable.zip (Observable.zip a b) c
        |> Observable.map (fun ((a, b), c) -> a, b, c)
    ...
    type Shape<'A, 'S when 'A :> ShapeAttr<'A> and 'S :> Shape<'A, 'S>>
        (attrs: 'A seq, style: StyleAttrs seq, events: ShapeEvent<'S> seq)
        =
        ...
        let redraw () = updateTick |> Store.modify ((+) 1)

        let update (attrs: 'A seq) (style: StyleAttrs seq) (events: ShapeEvent<'S> seq) =
            let m1: Map<string, SutilElement> =
                attrs |> Seq.fold (fun s a -> s.Add(a.Name, a.Element)) attrMap

            let m2: Map<string, SutilElement> =
                style |> Seq.fold (fun s a -> s.Add(a.Name, a.Element)) m1
        ...
        default _.Update(attrs: 'A seq, style: StyleAttrs seq, events: ShapeEvent<'S> seq) =
        ...
        interface Updatable<'A, ShapeEvent<'S>> with
            member _.Update(attrs: 'A seq, style: StyleAttrs seq, events: ShapeEvent<'S> seq) =
                update attrs style events
    ...
    let interp v a b c d = ((v - a) / b) * (d - c) + c
    ...
    type Group(attrs: GroupAttrs seq, style: StyleAttrs seq, events: ShapeEvent<Group> seq) =
        ...
        member __.Children(children: Viewable seq) =
            _children <- children |> Seq.toArray
            __.Redraw()
            __
                    ...
                    Bind.el (
                        ...
                        fun _ ->
                            fragment [
                                ...
                                yield! _children |> Array.map (fun c -> c.View())
                            ]
                    )
                ]
    ...
    type CircleTransition(circle: Circle) =
        ...
        member __.Duration(d: int) =
            duration <- d
            __

        member __.Delay(d: int) =
            delay <- d
            __
        ...
        override __.Update(attrs, style, events) =
            ...
            |> Seq.iter (fun a ->
                DomHelpers.timeout
                    (fun () -> transition circle (a.AsFloat) duration (a.AsCtor))
                    delay
    ...
    type RectTransition(rect: Rect) =
        ...
        override __.Update(attrs, style, events) =
            ...
            |> Seq.iter (fun a ->
                DomHelpers.timeout
                    (fun () -> transition rect (a.AsFloat) duration (a.AsCtor))
                    delay
                |> ignore // TODO: Save this so that we can cancel if a new transition occurs?
            )
    ...
    type GroupTransition(group: Group) =
        ...
        override __.Update(attrs, style, events) =
            ...
            |> Seq.iter (fun a ->
                DomHelpers.timeout
                    (fun () -> transition group (a.AsFloat) duration (a.AsCtor))
                    delay
                |> ignore // TODO: Save this so that we can cancel if a new transition occurs?
            )
...
let view () =
    ...
    let rec trans0 (c: Circle) =
        c.Transition().Duration(400).R(10).Cx(350).OnClick(trans2).End()

    and trans2 (c: Circle) =
        c.Transition().Duration(400).R(100).Cx(200).OnClick(trans0).End()

    let c1 = Circle()
    let c2 = Circle()

    c2.Cx(100).Cy(200).R(50).Fill("lime").Stroke("black").StrokeWidth(2.0).End()
    ...
    let rec collapseCard (r: Rect) =
        r.OnClick(expandCard).Transition().Duration(200).Height(20).End()

    and expandCard (r: Rect) =
        r.OnClick(collapseCard).Transition().Duration(200).Height(cardHeight).End()
    ...
    card
        ...
        .Stroke("gray")
        .StrokeWidth(1)
        .OnClick(fun r -> r.ToggleClass("collapsed").End())
        .End()
