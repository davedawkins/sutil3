...
module ApiDoc =
    ...
    let parseMember (memberEl: XmlElement) =
        ...
        let fullName, parms =
            ...
            if paren < 0 then
                ...
                fullNameArgs.Substring(0, paren), fullNameArgs.Substring(paren)
    ...
    let parseMembers (membersEl: XmlElement) =
        membersEl |> getElements "member" |> List.map parseMember
    ...
    let generateApi (doc: XmlElement) =
        let api =
            {
                AssemblyName = doc |> getElement "assembly" |> getElement "name" |> getText
                Members = doc |> getElement "members" |> parseMembers
            }

        let e =
            elc "div" "api-docs" [
                elc "h1" "assembly-name" [
                    text api.AssemblyName
                ]
