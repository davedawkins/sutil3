...
let view () : SutilElement =
    ...
    withStyle styleSheet <| Html.divc "board" [
        ...
        Html.divc "complete-all-container" [
            Bind.el(model,fun m -> Html.a [
                Attr.href "#"
                text "toggle sort"
                Ev.onClick (fun e -> e.preventDefault(); not m.Sort |> SetSort |> dispatch)
            ])
            Html.a [
                Attr.href "#"
                text "complete all"
                Ev.onClick (fun e -> e.preventDefault(); dispatch CompleteAll)
            ]
            Html.spanc "kudos" [
