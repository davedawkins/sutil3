...
type VirtualElement with
    ...
    member __.MapContext (ctx) = __.BuildMappers |> Array.fold ((|>)) ctx

    member __.AddBuildMapper( mapper : BuildContext -> BuildContext ) = { __ with BuildMappers = Array.singleton mapper |> Array.append __.BuildMappers }
    ...
    member __.AddEvent( name, handler, ?options : Internal.CustomEvents.EventOption[] ) = 
        { __ with Events = Array.singleton (name,handler, options |> Option.defaultValue Array.empty) |> Array.append __.Events }
    ...
    member __.AsString( children ) = 
        let attrs =
            if __.Attributes.Length = 0 
                then "" 
                else " " + (__.Attributes |> Array.map (fun (n,v) -> sprintf "%s=\"%A\"" n v) |> String.concat " ")
        ...
        |> (fun s -> if __.BuildMappers.Length > 0 then "MAPPER(" + s + ")" else s)
...
let rec addSutilElement (parent : VirtualElement) ( se : SutilElement ) : VirtualElement =
    ...
    | Event (name, handler, options) ->
        parent.AddEvent(name,handler,options)
            ...
            |> _.AddEvent( 
                ...
                fun e ->
                    ...
                    let ctx : BuildContext =
                        JsMap.getKey el "__sutil_ctx"

                    if isNull (ctx :> obj) then
                        Log.Console.error("Key '__sutil_ctx' not set on ", el |> DomHelpers.toStringSummary )
                    else
                        if _log.enabled then
                            _log.trace("Mount: init '", name , "'", e.target.asElement |> Internal.DomHelpers.toString)
    ...
    | BuildMap (map, child) ->
        (addSutilElement parent child).AddBuildMapper map
...
and fromSutil (se : SutilElement) : VirtualElement =
    ...
    let el = 
        ...
        if root.Children.Length = 1 && noAttributes then   
            root.BuildMappers |> Array.fold (fun (ve : VirtualElement) mapper -> ve.AddBuildMapper(mapper))  root.Children[0]
...
let rec toDom (_context : BuildContext) (ve : VirtualElement) : Browser.Types.Node  =
    ...
    | TextNode s -> 
        if _log.enabled then _log.trace("toDom: TextNode", s, context.ParentNode |> Internal.DomHelpers.toStringSummary )
        DomEdit.text s

    | TagNode tag ->
        if _log.enabled then _log.trace("toDom: TagNode", "<" + tag + "> parented to", context.ParentNode |> Internal.DomHelpers.toStringSummary )
        let el = context.CreateElement tag

        let _id = context.NextId()
        if _log.enabled then _log.trace("toDom: -- setSutilId", _id)

        Id.setId el (_id |> string)

        if (ve.Attributes |> Array.exists( fun (name, value) -> name = "data-binding")) then
            if _log.enabled then _log.trace("toDom: -- set __sutil_ctx ", el |> DomHelpers.toStringSummary)

            JsMap.setKey el "__sutil_ctx" _context 

        ve.Attributes 
            |> Array.iter (fun (name,value) -> 
                if _log.enabled then _log.trace("toDom: -- setAttribute", "'" + name + "'", value)
                DomEdit.setAttribute el name value )
        ve.Events 
            |> Array.iter (fun (name,handler,options) -> 
                if _log.enabled then _log.trace("toDom: -- addEventListener", "'" + name + "'" )
                if options |> Array.exists (function Internal.CustomEvents.Once -> true) then
        ...
        |> Array.iter (fun child -> 
            let childEl : Browser.Types.Node = 
                if _log.enabled then _log.trace("toDom: -- addChild to ", el |> Internal.DomHelpers.toStringSummary )
                toDom 
                    (context
                        .WithParent(el)
    ...
    | SideEffectNode (name, effect) ->
        if _log.enabled then _log.trace("toDom: Effect", name, context.ParentNode |> Internal.DomHelpers.toStringSummary )
