...
let rec private calculatePatch (existing : Node) (ve : VElement) : Action =
    ...
    if isTextNode existing && ve.IsTextNode then
        if existing.textContent <> ve.InnerText then
            Patch ( ve,  [| SetInnerText ve.InnerText |] )
        else 
            AsIs
        ...
        |]
        |> (fun patches -> if patches.Length > 0 then Patch (ve, patches) else AsIs)
        ...
        if ctx.LogPatchEnabled then
            _log.trace( 
                sprintf "Patch action:\n current= %s\n action=%s\nparent=%s" 
                    ...
                    (patch.ToString())
                    (if isNull existing then "" else existing.parentElement |> DomHelpers.toStringSummary)
                    )
...
let rec private applyPatchAction (context : BuildContext) (a : PatchAction) : PatchResult =
    ...
    | SetAttr (name,value) -> 
        ...
        if _log.enabled then _log.trace("SetAttr: ", (current |> Internal.DomHelpers.toStringSummary), name, value)

        DomEdit.setAttribute (asElement current) name value
        AttrSet

    | RemoveAttr (name,_) -> 
        if _log.enabled then _log.trace("RemoveAttr: ", (current |> Internal.DomHelpers.toStringSummary), name)
        DomEdit.removeAttribute (asElement current) name
        AttrRemoved

    | AddEvent(name,value,options) -> 
        if _log.enabled then _log.trace("AddEvent: ", (current |> Internal.DomHelpers.toStringSummary), name)

        if options |> Array.exists (function Internal.CustomEvents.Once -> true | _ -> false) then
    ...
    | RemoveEvent (name,value) -> 
        if _log.enabled then _log.trace("RemoveEvent: ", (current |> Internal.DomHelpers.toStringSummary), name)
        (asElement current).removeEventListener(name,value)
        EventRemoved

    | SetInnerText text ->
        if _log.enabled then _log.trace("SetInnerText: ", (current |> Internal.DomHelpers.toStringSummary), text)
        current.textContent <- text
        TextSet

    | ChildAction (ix, action) ->
        if _log.enabled then _log.trace("Child: ", ix, nodeChild ix |> Internal.DomHelpers.toStringSummary)
        let childResult =
            applyAction 
    ...
    | ApplyEffect ve ->
        ...
        | SideEffectNode (name, effect) ->
            ...
            if _log.enabled then _log.trace("Apply Effect: ", (current |> Internal.DomHelpers.toStringSummary), name)
...
and  private applyAction (context : BuildContext) (current : Node) (action: Action) : SutilResult =
    ...
    match action with
    | AsIs ->
        (Unchanged, current) |> SutilResult.Of

    | Remove ->
        if _log.enabled then _log.trace("Remove: ", (current |> Internal.DomHelpers.toStringSummary))
        DomEdit.remove current
        (Removed, current) |> SutilResult.Of

    | ReplaceNew (ve) ->
        let de = VirtualDom.toDom context ve
        if _log.enabled then _log.trace("Replace: ", (current |> Internal.DomHelpers.toStringSummary), " with ", (de |> DomHelpers.toStringSummary))
    ...
    | Patch (ve, patches) ->
        if _log.enabled then _log.trace("Patch: ", (current |> Internal.DomHelpers.toStringSummary))
        if not (isNull current) then 
            Dispose.disposeNode (current) // Cleanup event listeners and disposables
        else 
            if _log.enabled then _log.trace("Patch: current is null")

        let result = 
            patches 
            |> Array.map (applyPatchAction (context.WithParent(current).WithAppendNode(DomEdit.append) |> ve.MapContext))
