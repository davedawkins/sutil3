...
type SutilHtmlEngine() as this =
    inherit HtmlEngine<SutilElement>( 
                _element, 
                _text, 
                (fun () -> _fragment []) 
            )
    ...
    member _.text (v : IObservable<int>) = Bind.el (v .> string |> Store.distinct, _text)
    member _.text (v : IObservable<float>) = Bind.el (v  .> string |> Store.distinct, _text)
...
type SutilAttrEngine() =
    inherit AttrEngine<SutilElement>((fun key value -> _attr(key, string value)),
